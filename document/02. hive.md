...
6. 원본 테이블과 파티션된 테이블과의 쿼리 실행 속도 비교

> SELECT day_of_week, COUNT(1) FROM ACCIDENTS WHERE SUBSTR(Date_Acc, 7) = 2015 AND SUBSTR(Date_Acc,4,2) = 06 GROUP BY day_of_week;
